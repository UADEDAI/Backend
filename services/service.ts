...
const defaultHealthCheckTimeout = +(
  process.env.SERVICE_HEALTH_CHECK_TIMEOUT || 5000
);
const defaultConnectionTimeout = +(
  process.env.SERVICE_CONNECTION_TIMEOUT || 21000
);
...
export class Service {
      ...
      return this.r
        ...
        .catch((error) => {
          ...
          if (error.response) {
             ...
             * Convert axios error to http-errors
            ...
            return reject(
              new HttpException(
                {
                  message: 'Timeout Error. Try again later.',
                  extra: {
                    ...extra,
                    code: 'REQUEST_ERROR_TIMEOUT',
                    rawCode: error.code,
                  },
                },
  ...
  health(options: any = {}) {
    options.url =
      options.url || options.uri || this.options.health || '/health';
