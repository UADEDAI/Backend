...
export abstract class Model<T> extends BaseModel<T> {
  ...
  static async resolveIncludes(model: any, findOptions: any) {
    ...
    for (const includeOptions of findOptions.include) {
      ...
      if (related && includeOptions.model.afterFindHook) {
        await includeOptions.model.afterFindHook(
          related,
          relations[include] || {},
        );
      }
  ...
  static async resolveRemotes(model: any, findOptions: any) {
    for (const remote of findOptions.remotes) {
      const remoteOptions: IRemoteOptions = Reflect.getMetadata(
        `remote_${remote}`,
        model,
        remote,
      );
  ...
  static async resolveRemote(
    ...
    uri = resolvePossibleFunctionValue(
      uri,
      `/api/v1/${relationship}`,
      args,
    ) as string;
    ...
    // 03. If URI is null, cancel this population
    if (!uri) {
      // 03. a. Check if this attribute is required
      if (resolvePossibleFunctionValue(required, true, args)) {
        throw new Error(
          `Cant load remote value for ${remoteField} from ${method} ${uri} with data ${JSON.stringify(
            options,
          )}`,
        );
      }
    ...
    // 04. Try to call endpoint with resolved variables
    try {
      const serviceOptions: any = {
        uri,
        ...(await Model.resolveRemoteRequestOptions(
          relations[remoteField],
          options,
        )),
      };

      await onBeforeCall?.(serviceOptions, model);
      const result = await service[method](serviceOptions);
      await onAfterCall?.(result, model);

      model.setDataValue(
        remoteField,
        onResult ? await onResult?.(result) : result,
      );
    } catch (e) {
      // 04. a. Check if this attribute is required
      if (resolvePossibleFunctionValue(required, true, args)) {
        logger.error(
          `Cant load remote value for ${remoteField} from ${method} ${uri} with data ${JSON.stringify(
            options,
      ...
      // 04. b. Check if is necesary debug warning when call fail
   ...
   */
  static async resolveRemoteRequestOptions(
    remoteOptions: any,
    defaultOptions: any,
  ) {
    let defaultInclude;
    ...
    if (remoteOptions) {
      includes = await Model.resolveRemoteRequestOptionsForIncludes(
        remoteOptions,
      );
      remotes = await Model.resolveRemoteRequestOptionsForRemotes(
        remoteOptions,
      );
    }
    ...
    if (includes && includes.length) {
      defaultInclude = defaultOptions.params.include
        ? defaultOptions.params.include.split(',')
        : [];
      defaultOptions.params.include = defaultInclude.concat(includes).join(',');
    }
    if (remotes && remotes.length) {
      const defaultRemotes = defaultOptions.params.remotes
        ? defaultOptions.params.remotes.split(',')
        : [];
      defaultOptions.params.include = defaultRemotes
        .concat(remotes)
   ...
   */
  static async resolveRemoteRequestOptionsForIncludes(
    remoteOptions: any,
   ...
   */
  static async resolveRemoteRequestOptionsForRemotes(
    remoteOptions: any,
    parentField: string = null,
  ) {
    const prefixKey = parentField ? `${parentField}.` : '';
